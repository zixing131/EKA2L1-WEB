...
std::unique_ptr<std::thread> os_thread_obj_hos;
std::unique_ptr<std::thread> gr_thread_obj_hos;
...
namespace eka2l1::hos {
    ...
    static int graphics_driver_thread_initialization(emulator &state) {
        ...
        state.window = std::make_unique<drivers::emu_window_ohos>();
        state.window->init("EKA2L1", eka2l1::vec2(0, 0),
            drivers::emu_window_flag_maximum_size);
        state.window->set_userdata(&state);

        // We got window and context ready (OpenGL ES via EGL). Make graphics now.
        ...
        drivers::emu_window_ohos *window = state.window.get();
    ...
    void graphics_driver_thread(emulator &state) {
        ...
        // Keep running. Switching graphics backend requires restarting EKA2L1.
    ...
    bool emulator_entry(emulator &state) {
        ...
        // Instantiate OS thread.
        if (result) {
            os_thread_obj_hos = std::make_unique<std::thread>(os_thread, std::ref(state));
        }

        // Run graphics driver on its own thread.
        state.graphics_init_done.reset();
        gr_thread_obj_hos = std::make_unique<std::thread>(graphics_driver_thread, std::ref(state));
