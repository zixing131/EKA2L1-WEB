...
TEST_CASE("linked fonts skip ROM CJK placeholder glyphs", "[fbs][web]") {
    fake_font_adapter rom({ 'A', 0x4E2D }, 6);
    fake_font_adapter fallback({ 0x4E2D }, 12);
    epoc::open_font_face_attrib attrib{};
    epoc::adapter::linked_font_file_adapter linked({ { &rom, 0, true }, { &fallback, 0 } }, 0, attrib);

    REQUIRE(linked.get_glyph_advance(0, 'A', 0, false) == 6);
    REQUIRE(linked.get_glyph_advance(0, 0x4E2D, 0, false) == 12);
    REQUIRE(linked.has_character(0, 0x4E2D, 0));

    epoc::adapter::linked_font_file_adapter placeholder_only({ { &rom, 0, true } }, 0, attrib);
    REQUIRE(placeholder_only.has_character(0, 'A', 0));
    REQUIRE_FALSE(placeholder_only.has_character(0, 0x4E2D, 0));
}

TEST_CASE("FreeType expands monochrome strikes for antialiased guest fonts", "[fbs][web]") {
    const std::string bdf =
        ...
        "STARTCHAR A\nENCODING 65\nSWIDTH 1000 0\nDWIDTH 8 0\nBBX 8 8 0 0\n"
        "BITMAP\n18\n24\n42\n7E\n42\n42\n42\n00\nENDCHAR\nENDFONT\n";
    std::vector<std::uint8_t> data(bdf.begin(), bdf.end());
    auto adapter = epoc::adapter::make_font_file_adapter(epoc::adapter::font_file_adapter_kind::freetype, data);
    REQUIRE(adapter);
    REQUIRE(adapter->is_valid());

    epoc::glyph_bitmap_type type = epoc::antialised_glyph_bitmap;
    epoc::open_font_character_metric metric{};
    int width = 0, height = 0;
    std::uint32_t size = 0;
    auto bitmap = adapter->get_glyph_bitmap(0, 'A', 8, &width, &height, size, &type, metric);
    REQUIRE(bitmap);
    REQUIRE(type == epoc::antialised_glyph_bitmap);
    REQUIRE(metric.bitmap_type == type);
    REQUIRE(width == 8);
    REQUIRE(height == 8);
    REQUIRE(size == 64);
    for (int x = 0; x < width; x++) {
        REQUIRE(bitmap[x] == ((x == 3 || x == 4) ? 255 : 0));
    }
