...
if [ -n "${KEYSTORE:-}" ]; then
    if [ -z "${KEY_ALIAS:-}" ] || [ -z "${KEY_PASS:-}" ] || [ -z "${STORE_PASS:-}" ]; then
        warn "KEYSTORE 已设置，但 KEY_ALIAS / KEY_PASS / STORE_PASS 未完整提供，跳过签名，输出未签名版本。"
        cp "$APK_FILE" "$FINAL_APK"
    else
        APKSIGNER="$ANDROID_SDK_ROOT/build-tools/$(ls "$ANDROID_SDK_ROOT/build-tools" | sort -V | tail -1)/apksigner"
        if [ ! -f "$APKSIGNER" ]; then
            warn "未找到 apksigner，跳过签名，输出未签名版本。"
            cp "$APK_FILE" "$FINAL_APK"
        else
            info "正在签名 -> $FINAL_APK"
            "$APKSIGNER" sign \
    ...
    warn "未设置 KEYSTORE，输出未签名 APK。安装前需手动签名。"
    cp "$APK_FILE" "$FINAL_APK"
fi
