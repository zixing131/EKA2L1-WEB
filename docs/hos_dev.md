...
## 6.10 第八轮 — 黑屏(有 FPS)真正根因:EGLSurface 创建失败(EGL_BAD_MATCH)

§6.8 当时只拿到「HAP 没带着色器」一个面;把着色器打进 HAP 后**仍然黑屏(有 FPS)**。这次
抓到了 hilog,直接推翻了「缺着色器」假设:

```
higpu          W  [nodict]EGL_BAD_MATCH:pixel format (surface:... config:...)
OpenGLWrapper  E  egl.eglCreateWindowSurface error.
OpenGLWrapper  E  EGLSurface is invalid.
OpenGLWrapper  W  ThreadPrivateDataCtl::SetError error = 12301.   (EGL_BAD_SURFACE 0x300D)
```

### 根因:EGLConfig 没选 alpha,和 XComponent 的 RGBA8888 窗口格式不匹配

`eglCreateWindowSurface` 报 `EGL_BAD_MATCH: pixel format` → surface 创建失败 → 之后每次
draw/swap 都是 `EGLSurface is invalid`(`EGL_BAD_SURFACE` 12301)→ 全黑。FPS 照跑是因为
渲染循环/命令队列没停,只是 swap 全失败。

[context_egl.cpp](src/emu/drivers/src/graphics/backend/context_egl.cpp) 的 config 属性只要了
`EGL_RED/GREEN/BLUE_SIZE=8`、**没要 `EGL_ALPHA_SIZE`**,于是可能选到 RGB(X)888 的 config;
而 XComponent 的 `OHNativeWindow` 默认是 **RGBA8888**。两者像素格式不一致 → BAD_MATCH。

**Android 早就处理了这件事**([context_egl_android.cpp](src/emu/drivers/src/graphics/backend/context_egl_android.cpp)):
创建 surface 前取 config 的 `EGL_NATIVE_VISUAL_ID`,用
`ANativeWindow_setBuffersGeometry(window, 0, 0, format)` 把窗口 buffer 格式改成和 config 一致。
而 [context_egl_ohos.cpp](src/emu/drivers/src/graphics/backend/context_egl_ohos.cpp) 当初**故意
跳过了这步**(注释说"XComponent 已配置好格式,不用管")——这个假设就是 bug。

### 修复:OHOS 做 Android 的等价操作(SET_FORMAT)

`context_egl_ohos::create_surface()`:取 `EGL_NATIVE_VISUAL_ID`,调
`OH_NativeWindow_NativeWindowHandleOpt(window, SET_FORMAT, format)`(头
`<native_window/external_window.h>`)把 native window 格式设成和 EGLConfig 一致,再创建
surface。`drivers` 库补链 `native_window`(`libnative_window.so`,之前只 entry 链了)。

> 顺带(§6.8 的真 bug,本轮一并修):`Emulator.ets` 的 `copyAssets` 原来用「目录在不在」当
> gate,而早期没带着色器的 HAP 已经建出空 `resources/` 目录,导致后来带着色器的 HAP 永远跳过
> 拷贝→沙箱里着色器永远缺。改成**版本戳**(`.assets_version`)gate:版本变了就重拷(且写文件
> 用 `TRUNC` 覆盖旧的空文件)。这个 bug 在 EGL 修好、真正要读着色器时才会现形,先一并堵上。

### 教训

1. **「黑屏 + 有 FPS」要先抓 GPU/EGL 日志**:渲染循环在转不代表 surface 有效。`EGL_BAD_MATCH`
   /`EGL_BAD_SURFACE`(12301)是 surface 创建/绑定失败的典型码,跟着色器、资源八竿子打不着。
   我先前按「缺着色器」推是因为没日志;一条 hilog 就锁定了真因。**有日志再下结论。**
2. **EGLConfig 必须和 native window 的像素格式对齐**:Android/OHOS 的窗口默认带 alpha
   (RGBA8888),config 不要 alpha 就可能 BAD_MATCH。要么 config 加 `EGL_ALPHA_SIZE=8`,要么
   照平台惯例把窗口格式设成 config 的 `EGL_NATIVE_VISUAL_ID`(本轮取后者,OHOS 对齐 Android)。
3. **移植时「平台已经替我配好了」的注释要存疑**:OHOS 版当初假设 XComponent 配好了格式而省掉
   setBuffersGeometry 等价步骤,正是黑屏根因。Android 做了的平台适配,OHOS 多半也要做。
...
**第八轮真机修复（§6.10，黑屏有 FPS = EGLSurface 创建失败 BAD_MATCH）**
| 文件 | 改动 |
|---|---|
| `src/emu/drivers/src/graphics/backend/context_egl_ohos.cpp` | `create_surface` 前用 `OH_NativeWindow_NativeWindowHandleOpt(SET_FORMAT, EGL_NATIVE_VISUAL_ID)` 对齐窗口格式（对标 Android setBuffersGeometry） |
| `src/emu/drivers/CMakeLists.txt` | OHOS 的 `drivers` 补链 `native_window` |
| `src/emu/hos/entry/src/main/ets/service/Emulator.ets` | `copyAssets` 改用版本戳 gate（修空目录导致永不重拷的着色器缺失隐患）；写文件加 `TRUNC` |
